...
void Initialize(long baud)
{
    ...
    ModbusSerial.begin(baud); //, SERIAL_8E1); pass serial format (8 wide, Even parity, 1 Stop bit) if you can (SerialUSB doesn't support it)
...
void Process(Registers &registers)
{
    ...
    while (ModbusSerial.available() > 0)
    {
        ...
        {
            // ModbusSerial.read(); //use this instead if your Serial doesn't support .clear()
            ModbusSerial.clear();
        }
